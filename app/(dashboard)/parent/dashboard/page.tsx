...
interface Sibling { id: string; first_name: string; last_name: string; class_name: string; roll_number: string; gr_number: string; }
interface ClassTeacher { first_name: string; last_name: string; phone: string; email: string; }
interface DashData {
  student: { id: string; first_name: string; last_name: string; roll_number: string; gr_number: string; class_name: string; parent_name: string; };
...
export default function ParentDashboard() {
  ...
  // Initial load to get siblings list
  useEffect(() => { loadData(''); }, []);
  // Reload when child or month changes
  useEffect(() => { if (selChild) loadData(selChild); }, [selChild, selMonth]);

  const loadData = async (childId: string) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ month: selMonth });
      if (childId) params.set('child_id', childId);
      const res = await fetch(`/api/student/dashboard-data?${params}`);
      if (res.status === 401) { router.push('/student-login'); return; }
      if (!res.ok) throw new Error('Failed');
      const json = await res.json();
      // Redirect students away from parent dashboard
      if (json.role !== 'parent') { router.push('/student/dashboard'); return; }
      setData(json);
      // Set default child on first load
      if (!selChild) {
        setSelChild(json.student.id);
      }
    } catch (err) {
  ...
  if (loading && !data) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-sm text-slate-500">Loading parent portal...</p>
      </div>
    </div>
  );
  ...
  const { student: stu, cumulative: cum, monthly: mon, subjectAtt, academicYear: ay, classTeacher, siblings } = data;
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      <div className="bg-gradient-to-r from-teal-600 via-cyan-600 to-blue-600 text-white">
        <div className="max-w-5xl mx-auto px-4 py-5 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ fontFamily: 'var(--font-display)' }}>👨‍👩‍👧 Parent Portal</h1>
            <p className="text-sm text-white/70">{stu.parent_name || `${stu.first_name}'s Parent`}</p>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        {siblings.length > 1 && (
          <div className="bg-white rounded-2xl border border-slate-100 p-4 mb-6 shadow-sm">
            <p className="text-xs font-medium text-slate-500 mb-2">Select Child</p>
            <div className="flex gap-2 flex-wrap">
              {siblings.map(c => (
                <button key={c.id} onClick={() => setSelChild(c.id)}
                  className={`px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${selChild === c.id ? 'bg-gradient-to-r from-teal-600 to-blue-600 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                  {c.first_name} {c.last_name} <span className="opacity-70">· {c.class_name}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* Active Child Info */}
        <div className="bg-white rounded-2xl border border-slate-100 p-4 mb-6 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-lg font-bold text-slate-800">{stu.first_name} {stu.last_name}</p>
            <p className="text-sm text-slate-500">{stu.class_name} · Roll No. {stu.roll_number} · GR: {stu.gr_number}</p>
          </div>
          {cum.workingDays > 0 && (
            <span className={`text-2xl font-bold px-4 py-2 rounded-xl ${cum.pct >= 75 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>{cum.pct}%</span>
          )}
        </div>
        ...
        {cum.workingDays > 0 && cum.pct < 75 && (
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4 mb-6 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-red-800">Low Attendance Alert for {stu.first_name}!</p>
              <p className="text-xs text-red-600">Attendance is <strong>{cum.pct}%</strong> — below the required 75%. Please ensure regular attendance.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
            ...
            <p className="text-[10px] text-slate-400">{cum.present}/{cum.workingDays} days</p>
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
            <p className="text-xs text-slate-500 mb-1">This Month</p>
            <p className={`text-3xl font-bold ${mon.pct >= 75 ? 'text-blue-600' : 'text-red-600'}`}>{mon.pct}%</p>
            <p className="text-[10px] text-slate-400">{mon.present}/{mon.workingDays} days</p>
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
            <p className="text-xs text-slate-500 mb-1">Days Absent</p>
            <p className="text-3xl font-bold text-red-500">{cum.absent}</p>
            <p className="text-[10px] text-slate-400">of {cum.workingDays} working</p>
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
            <p className="text-xs text-slate-500 mb-1">Status</p>
            <p className="text-2xl font-bold">{cum.pct >= 90 ? '🟢' : cum.pct >= 75 ? '🟡' : '🔴'}</p>
            <p className="text-[10px] text-slate-400">{cum.pct >= 75 ? 'Good' : 'Needs attention'}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-bold text-slate-700">Attendance Progress</span>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-6 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {mon.days.map(d => (
              <div key={d.day} className={`aspect-square flex flex-col items-center justify-center rounded-xl text-xs font-bold ${statusColor(d.status)}`}>
        ...
        {subjectAtt.length > 0 && (
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-6 shadow-sm">
            <h3 className="text-sm font-bold text-slate-700 mb-4">Subject-wise Attendance</h3>
        ...
        {classTeacher && (
          <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-2xl border border-teal-200 p-5 mb-6">
            <h3 className="text-sm font-bold text-teal-800 mb-3">📞 Class Teacher</h3>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-slate-800">{classTeacher.first_name} {classTeacher.last_name}</p>
                {classTeacher.phone && <p className="text-xs text-slate-500 mt-1">📱 {classTeacher.phone}</p>}
                {classTeacher.email && <p className="text-xs text-slate-500">📧 {classTeacher.email}</p>}
              </div>
              {classTeacher.phone && (
                <a href={`tel:${classTeacher.phone}`} className="px-4 py-2 bg-teal-600 text-white rounded-xl text-sm font-semibold hover:bg-teal-700 transition-all">📞 Call</a>
              )}
            </div>
          </div>
        )}
        ...
        <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-5 text-white">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-white/10 rounded-xl p-3"><p className="text-xs text-white/60">Class</p><p className="font-bold">{stu.class_name}</p></div>
            ...
            <div className="bg-white/10 rounded-xl p-3"><p className="text-xs text-white/60">GR Number</p><p className="font-bold">{stu.gr_number}</p></div>
          </div>
        </div>
      </div>
    </div>
